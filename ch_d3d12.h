...
#ifndef CH_ARRAY_COUNT
#define CH_ARRAY_COUNT(Array) (sizeof(Array)/sizeof((Array)[0]))
#endif

namespace ch
{
#define CH_DXOP(Value) CH_ASSERT(SUCCEEDED(Value))
        ...
        InitFrameContext(ID3D12Device *D)
    {
        ...
        CH_DXOP(D->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                          IID_PPV_ARGS(&Context.CmdAllocator)));
        
        CH_DXOP(D->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&Context.Fence)));
        ...
        InitGPUContext(ID3D12Device *D, int FramesInFlight)
    {
        ...
        CH_DXOP(D->CreateCommandQueue(&CmdQueueDesc, IID_PPV_ARGS(&Context.CmdQueue)));
        ...
        CH_DXOP(D->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                     Context.Frames[0].CmdAllocator, 0,
                                     IID_PPV_ARGS(&Context.CmdList)));
        CH_DXOP(Context.CmdList->Close());
        ...
        gpu_context::Reset(UINT CurrentBackbufferIndex)
    {
        ...
        CH_DXOP(Frames[BI].CmdAllocator->Reset());
        CH_DXOP(CmdList->Reset(Frames[BI].CmdAllocator, 0));
    }
        ...
        gpu_context::WaitForGpu(UINT NextBI)
    {
        ...
        CH_DXOP(CmdQueue->Signal(Frames[BI].Fence, Frames[BI].FenceValue));
        
        frame_context *NextFrame = Frames + NextBI;
        
        if (NextFrame->Fence->GetCompletedValue() < NextFrame->FenceValue)
        {
            CH_DXOP(NextFrame->Fence->SetEventOnCompletion(NextFrame->FenceValue, NextFrame->FenceEvent));
        ...
        gpu_context::FlushFramesInFlight()
    {
        for (int BufferI = 0; BufferI < FramesInFlight; ++BufferI)
        {
            ...
            {
                CH_DXOP(Frame->Fence->SetEventOnCompletion(Frame->FenceValue, Frame->FenceEvent));
        ...
        gpu_context::_PushToBarrierCache(D3D12_RESOURCE_BARRIER Barrier)
    {
        ...
        if (CachedBarrierCount == CH_ARRAY_COUNT(CachedBarriers))
        {
            FlushBarriers();
        }
    }
                        ...
                        int BackbufferCount)
    {
        ...
        CH_DXOP(CreateDXGIFactory2(0, IID_PPV_ARGS(&Factory2)));
        ...
        CH_DXOP(Factory2->CreateSwapChainForHwnd(CmdQueue, // for d3d12, must be command queue instead of device
                                                 Window,
                                                 &SwapChainDesc,
                                                 0, // doesn't support fullscreen
                                                 0,
                                                 &SwapChain1));
        CH_DXOP(SwapChain1->QueryInterface(IID_PPV_ARGS(&SwapChain)));
        
        //NOTE(chen): tells windows to get the fuck out of my business
        //            I will handle resize/fullscreen myself goddamn it
        CH_DXOP(Factory2->MakeWindowAssociation(Window, DXGI_MWA_NO_WINDOW_CHANGES|DXGI_MWA_NO_ALT_ENTER|DXGI_MWA_NO_PRINT_SCREEN));
                            ...
                            D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
    {
        ...
        CH_DXOP(D->CreateDescriptorHeap(&HeapDesc, IID_PPV_ARGS(&Arena.Heap)));
                             ...
                             ID3DBlob **RSErrorBlob_Out)
    {
        ...
        CH_DXOP(D3D12CreateRootSignatureDeserializer(CodeBlob->GetBufferPointer(),
        ...
        if (SUCCEEDED(D3D12SerializeRootSignature(RSDesc, 
                                                  ...
                                                  &RSBlob, &RSErrorBlob)))
        {
            CH_DXOP(D->CreateRootSignature(0, RSBlob->GetBufferPointer(),
    ...
    static pso
        InitComputePSO(ID3D12Device *D, char *Filename, 
                       char *EntryPoint, char *Target,
                       char *ErrorBuf_Out, size_t ErrorBufSize)
    {
        pso PSO = {};
        ...
        if (!File.Data)
        {
            if (ErrorBuf_Out)
            {
                snprintf(ErrorBuf_Out, ErrorBufSize, 
                         "Failed to load shader file: %s", Filename);
            }
            return PSO;
        }
        
        bool HasError = 0;
        ID3DBlob *Blob = CompileShader(File, Filename, EntryPoint, Target, &HasError);
        free(File.Data);
        if (HasError)
        {
            if (ErrorBuf_Out)
            {
                snprintf(ErrorBuf_Out, ErrorBufSize, 
                         "Shader compile error: %s", (char *)Blob->GetBufferPointer());
            }
            Blob->Release();
            return PSO;
        }
        ...
        if (!PSO.RootSignature)
        {
            ...
            {
                snprintf(ErrorBuf_Out, ErrorBufSize, 
                         "Root Signature serialization error: %s", ErrorMsg);
            }
            RSErrorBlob->Release();
        }
        ...
        CH_DXOP(D->CreateComputePipelineState(&PSODesc, IID_PPV_ARGS(&PSO.Handle)));
    ...
    static pso
        InitGraphicsPSO(ID3D12Device *D, char *Filename, 
                        char *VSEntryPoint, char *VSTarget, 
                        char *PSEntryPoint, char *PSTarget,
                        ...
                        edit_graphics_pso *EditGraphicsPSO = 0)
    {
        ...
        bool HasError = 0;
        ID3DBlob *VSBlob = CompileShader(File, Filename, VSEntryPoint, VSTarget, &HasError);
        if (HasError)
        {
            if (ErrorBuf_Out)
            {
                snprintf(ErrorBuf_Out, ErrorBufSize, 
                         "VS Shader compile error: %s", (char *)VSBlob->GetBufferPointer());
            }
            VSBlob->Release();
            return PSO;
        }
        
        ID3DBlob *PSBlob = CompileShader(File, Filename, PSEntryPoint, PSTarget, &HasError);
        if (HasError)
        {
            if (ErrorBuf_Out)
            {
                snprintf(ErrorBuf_Out, ErrorBufSize, 
                         "PS Shader compile error: %s", (char *)PSBlob->GetBufferPointer());
            }
            PSBlob->Release();
            return PSO;
        }
        ...
        CH_DXOP(D->CreateGraphicsPipelineState(&PSODesc, IID_PPV_ARGS(&PSO.Handle)));
                   ...
                   D3D12_RESOURCE_FLAGS Flags)
    {
        ...
        CH_DXOP(D->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE,
        ...
        Upload(ID3D12Resource *Buffer, void *Data, size_t ByteCount)
    {
        ...
        void *MappedAddr = 0;
        CH_DXOP(Buffer->Map(0, &ReadRange, &MappedAddr));
    ...
    int GetChannelCount(DXGI_FORMAT Format)
    {
        ...
        switch (Format)
        {
            ...
            default:
            {
                CH_ASSERT(!"unhandled DXGI format");
            } break;
        }
    ...
    void gpu_context::Upload(texture *Tex, void *TexData)
    {
        ...
        uint8_t *PaddedTexData = (uint8_t *)calloc(PaddedTexDataSize, 1);
        
        uint8_t *Reader = (uint8_t *)TexData;
        uint8_t *Writer = PaddedTexData;
        ...
        ch::Upload(StagingBuffer, PaddedTexData, PaddedTexDataSize);
